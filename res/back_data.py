...
if __name__ == "__main__":
    DPI = 20
    output_dir = f"res/plans/{DPI}"
    os.makedirs(output_dir, exist_ok=True)
    with open("res/ResPlan.pkl", "rb") as f:
        plans = pickle.load(f)
    processed_metadata, imgs = [], []
    for i, plan in enumerate(plans):
        if "balacony" in plan:
            plan["balcony"] = plan.pop("balacony")

        graph_obj = plan["graph"]
        nodes = list(graph_obj.nodes)
        edges = [(u, v, data["type"]) for u, v, data in graph_obj.edges(data=True)]
        ...
        img = render_plan_static(plan, dpi=DPI)
        imgs.append(img)
    ...
    imgs.tofile(f"{output_dir}/plan_imgs.bin")
